# Claude Code Orchestration Instructions
...
Always begin by acting as the Project Manager agent.
...
## How to Start
1. Read this file fully before taking any action.
2. Read all agent definitions in `.claude/agents/`.
3. Always begin by acting as the Project Manager agent.
4. Decompose the user's request into Task Briefs before doing any work.
5. Use sub-agents (Task() calls) for parallel workstreams where possible.

## Agent Files
All agent personas are in `.claude/agents/`:
- ProjectManager.md — coordinate all tasks (start here)
- SoftwareArchitect.md — system design
- UIUXDesigner.md — visual design + Nano Banana mockups
- SoftwareEngineer.md — implementation
- QAEngineer.md — testing
- SecurityReviewer.md — security audit
- DevOpsEngineer.md — deployment

## Environment
- FAL_API_KEY is in `.env` (never hard-code)
- See AGENTS.md for full tech stack and conventions
---
